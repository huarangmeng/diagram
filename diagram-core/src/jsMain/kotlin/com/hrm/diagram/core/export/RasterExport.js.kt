...
actual suspend fun RenderedDiagram.exportPng(
    ...
    if (!hasBrowserCanvas()) {
        return fallbackRasterExport(plan, "image/png", "JS raster export requires a browser canvas; returning fallback PNG bytes.")
    }
    ...
    return runCatching { render.canvas.toBlobBytes("image/png", 1.0) }.fold(
        ...
        onFailure = {
            fallbackRasterExport(plan, "image/png", "JS PNG export failed: ${it.message ?: it::class.simpleName}.")
        },
    )
}
...
actual suspend fun RenderedDiagram.exportJpeg(
    ...
    if (!hasBrowserCanvas()) {
        return fallbackRasterExport(plan, "image/jpeg", "JS raster export requires a browser canvas; returning fallback JPEG bytes.")
    }
    val render = renderBrowserCanvas(plan = plan)
    return runCatching { render.canvas.toBlobBytes("image/jpeg", options.quality.coerceIn(1, 100) / 100.0) }.fold(
        ...
        onFailure = {
            fallbackRasterExport(plan, "image/jpeg", "JS JPEG export failed: ${it.message ?: it::class.simpleName}.")
        },
    )
}
...
private fun renderCommands(
    ...
    for (command in commands.sortedBy { it.z }) {
        when (command) {
            ...
            is DrawCommand.DrawIcon -> diagnostics += exportWarning("DrawIcon raster fallback is not implemented on JS; icon '${command.name}' is omitted.")
...
private fun drawPath(ctx: CanvasRenderingContext2D, path: PathCmd) {
    ctx.beginPath()
    for (op in path.ops) {
        when (op) {
...
private fun roundedRectPath(ctx: CanvasRenderingContext2D, rect: com.hrm.diagram.core.draw.Rect, corner: Float) {
...
private fun applyStroke(ctx: CanvasRenderingContext2D, stroke: com.hrm.diagram.core.draw.Stroke, color: Color) {
    ctx.strokeStyle = color.toCssColor()
    ctx.lineWidth = stroke.width.toDouble()
    ctx.asDynamic().lineCap = when (stroke.cap) {
    ...
    ctx.asDynamic().lineJoin = when (stroke.join) {
    ...
    ctx.asDynamic().setLineDash((stroke.dash?.map { it.toDouble() } ?: emptyList()).toTypedArray())
}
...
private fun CanvasRenderingContext2D.measureTextWidth(text: String, font: com.hrm.diagram.core.draw.FontSpec): Double {
    save()
...
private suspend fun HTMLCanvasElement.toBlobBytes(type: String, quality: Double): ByteArray {
    ...
    return ByteArray(uint8.length) { index -> uint8.asDynamic()[index] as Byte }
}

private suspend fun Blob.readAsArrayBuffer(): ArrayBuffer =
...
private fun hasBrowserCanvas(): Boolean =
    js("typeof document !== 'undefined' && document != null && typeof document.createElement === 'function'") as Boolean
