...
/**
 * CSS color parser used by Mermaid style/theme parsing.
 *
 * Supported:
 * - Hex: #RGB, #RRGGBB, #AARRGGBB (kept for backward-compat with existing code paths)
 * - Named colors: full CSS Color Keywords (including `rebeccapurple`) + `transparent`
 * - rgb()/rgba(): comma-separated or space-separated, supports % channels, supports alpha as
 *   number (0..1) or percent.
 * - hsl()/hsla(): minimal support to cover common CSS usage (h in deg; s/l in %; alpha optional).
 *
 * NOTE: This is intentionally a small, dependency-free parser suitable for commonMain.
 */
internal object MermaidCssColors {
    fun parseToArgbIntOrNull(text: String): Int? {
        val s0 = text.trim().trim('\'', '"')
        if (s0.isEmpty()) return null
        val s = s0.lowercase()

        // Hex first.
        parseHexToArgbIntOrNull(s)?.let { return it }

        // Functional forms.
        if (s.startsWith("rgb(") || s.startsWith("rgba(")) return parseRgbLikeToArgbIntOrNull(s)
        if (s.startsWith("hsl(") || s.startsWith("hsla(")) return parseHslLikeToArgbIntOrNull(s)

        // Keywords.
        return CSS_KEYWORDS[s]
    }

    private fun parseHexToArgbIntOrNull(hex: String): Int? {
        if (!hex.startsWith("#")) return null
        val h = hex.substring(1)
        fun d(i: Int): Int = h[i].digitToIntOrNull(16) ?: return -1
        return when (h.length) {
            3 -> {
                val r = d(0); val g = d(1); val b = d(2)
                if (r < 0 || g < 0 || b < 0) return null
                val rr = r * 17; val gg = g * 17; val bb = b * 17
            ...
            8 -> {
                // Historical behavior: treat as AARRGGBB.
                (h.toLongOrNull(16) ?: return null).toInt()
            }
            else -> null
        }
    }

    private fun parseRgbLikeToArgbIntOrNull(s: String): Int? {
        val body = s.substringAfter('(', "").substringBeforeLast(')', "")
        if (body.isBlank()) return null

        // Normalize: allow either commas or whitespace. Also allow optional `/ alpha`.
        val hasComma = body.contains(',')
        val parts = if (hasComma) {
            body.split(',').map { it.trim() }
        } else {
            // Space-separated: split on whitespace, but keep '/' as a separator.
            body.replace("/", " / ").split(Regex("\\s+")).map { it.trim() }.filter { it.isNotEmpty() }
        }

        val (rgbParts, alphaPart) = splitAlpha(parts)
        if (rgbParts.size != 3) return null

        val r = parseRgbChannel(rgbParts[0]) ?: return null
        val g = parseRgbChannel(rgbParts[1]) ?: return null
        val b = parseRgbChannel(rgbParts[2]) ?: return null
        val a = alphaPart?.let { parseAlphaChannel(it) } ?: 255

        return (a shl 24) or (r shl 16) or (g shl 8) or b
    }

    private fun parseHslLikeToArgbIntOrNull(s: String): Int? {
        val body = s.substringAfter('(', "").substringBeforeLast(')', "")
        if (body.isBlank()) return null
        val hasComma = body.contains(',')
        val parts = if (hasComma) {
        ...
        val (hslParts, alphaPart) = splitAlpha(parts)
        if (hslParts.size != 3) return null

        val h = parseHueDegrees(hslParts[0]) ?: return null
        val sPct = parsePercent01(hslParts[1]) ?: return null
        val lPct = parsePercent01(hslParts[2]) ?: return null
        val a = alphaPart?.let { parseAlphaChannel(it) } ?: 255

        val (r, g, b) = hslToRgb(h, sPct, lPct)
        return (a shl 24) or (r shl 16) or (g shl 8) or b
    }

    private fun splitAlpha(parts: List<String>): Pair<List<String>, String?> {
        val slashIdx = parts.indexOf("/")
        if (slashIdx >= 0) {
            val rgb = parts.take(slashIdx)
            val alpha = parts.drop(slashIdx + 1).firstOrNull()
            return rgb to alpha
        }
        return if (parts.size == 4) parts.take(3) to parts[3] else parts to null
    }

    private fun parseRgbChannel(s: String): Int? {
        val t = s.trim()
        if (t.endsWith("%")) {
            val p = t.removeSuffix("%").trim().toFloatOrNull() ?: return null
            return clamp255((p / 100f) * 255f)
        }
        val v = t.toFloatOrNull() ?: return null
        return clamp255(v)
    }

    private fun parseAlphaChannel(s: String): Int? {
        val t = s.trim()
        if (t.endsWith("%")) {
            val p = t.removeSuffix("%").trim().toFloatOrNull() ?: return null
            return clamp255((p / 100f) * 255f)
        ...
        val v = t.toFloatOrNull() ?: return null
        // CSS alpha is 0..1 (but clamp to be tolerant).
        return clamp255(v.coerceIn(0f, 1f) * 255f)
    }

    private fun parseHueDegrees(s: String): Float? {
        val t = s.trim().lowercase()
        val raw = when {
            t.endsWith("deg") -> t.removeSuffix("deg").trim()
            t.endsWith("rad") -> {
                val rad = t.removeSuffix("rad").trim().toFloatOrNull() ?: return null
                return (rad * 180f / kotlin.math.PI.toFloat())
            }
            t.endsWith("turn") -> {
                val turn = t.removeSuffix("turn").trim().toFloatOrNull() ?: return null
                return turn * 360f
            }
            else -> t
        }
    ...
    private fun parsePercent01(s: String): Float? {
        val t = s.trim()
    ...
    private fun clamp255(v: Float): Int {
        ...
        return min(255, max(0, floor(vv + 0.5f).toInt()))
    }

    // Full CSS Color Keywords (CSS Color Module Level 4) + transparent.
    // Values are ARGB with alpha=0xFF unless otherwise noted.
    private val CSS_KEYWORDS: Map<String, Int> = mapOf(
