...
| PLANTUML-E003 | class 语法不匹配（class / package / member / relation / note 行，含 `end note` / `}` 配对错误） | class parser |
| PLANTUML-E004 | state 语法不匹配（state / transition / note / composite 行） | state parser |
| PLANTUML-E005 | component 语法不匹配（component/interface/port/database/queue、cluster、relation、note 行，含 `end note` / `}` 配对错误） | component parser |
| PLANTUML-E006 | usecase 语法不匹配（actor / usecase / cluster / relation / note 行，含 `end note` / `}` 配对错误） | usecase parser |
| PLANTUML-E007 | activity 语法不匹配（action / styled action / sync bar / legacy arrow / if / elseif / while / repeat / fork / swimlane / partition / note 行，含 `end note` / `}` 配对错误） | activity parser |
