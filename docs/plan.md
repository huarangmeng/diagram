...
### Phase 5 — PlantUML 扩展
timing、wireframe、archimate、c4、gantt、mindmap、wbs、network（✅ 已完成）、ditaa、json、yaml、chart（✅ 已完成）。
许多复用 Phase 1-3 的布局算法 + 新的 lowering 规则。

当前状态：🟡 已启动；`PlantUML mindmap` 最小可用链路已落地，当前已支持 `*` / `+` / `-` 层级、侧向继承、`: ... ;` 多行节点、stereotype、inline color、前导 `icon/emoji`（如 `<&flag>`、`<:smile:>`、`<#green:sunny:>`），以及 `<style>` 中基于 `BackgroundColor` / `LineColor` / `FontColor` / `RoundCorner` 的 tree style（`.class { ... }` / `.class * { ... }` + `<<class>>`），并完成 `@startmindmap` session 分发、`TreeIR + MindmapLayout` 渲染与 one-shot vs chunked 集成校验。`PlantUML wbs` 最小可用链路也已落地，当前支持 org-mode `*` 层级、算术前缀 `+/-`、`<` / `>` 方向改写、`_` boxless、`: ... ;` 多行节点、inline color、stereotype、前导 `icon/emoji`，以及同一套基于 `BackgroundColor` / `LineColor` / `FontColor` / `RoundCorner` 的 tree style，并完成 `@startwbs` session 分发、`TreeIR + MindmapLayout` 渲染与 one-shot vs chunked 集成校验。`PlantUML json/yaml` 链路已增强，当前支持 `@startjson/@endjson` 与 `@startyaml/@endyaml` 专用块、JSON object/array/scalar、YAML key-value/nested object/list/inline list、literal block scalar `|` 与 folded block scalar `>`，并通过 `StructIR + StructLayout` 渲染为嵌套结构行。✅ `PlantUML network (nwdiag)` 模块已完成，当前支持 `nwdiag {}`、`network {}`、`group {}`、`address`、节点属性与跨 network 同名节点连接，并通过 `GraphIR + Sugiyama` 渲染 network cluster 与 nested group cluster，已接入 `commonTest` 与 one-shot vs chunked 一致性校验。✅ `PlantUML timing` 模块已完成，当前支持 `clock/binary/concise/robust` track、`clock ... with period duration`、`duty N%` / `pulse duration` / `offset duration`、`hide time-axis`、`scale duration` / `scale duration as label`、`@time` / `@+duration` / `@time : label`、`ID is state` / `ID is state : text <<boundary>>` 区间、`A -> B : message` 与 `@10 <-> @50 : constraint`；`concise` 渲染为同一 lane 上的连续状态带与转场分隔线，并支持状态显示文本和 `<<dashed>>` / `<<thick>>` / `<<none>>` / `<<solid>>` 边界样式，`binary/clock` 渲染为高低电平 waveform，无显式 state 的 clock 会按 period 自动生成 low/high 翻转段，占空比与 offset 会生成非对称/延迟 waveform，robust 渲染为带浅底、粗边框、端点刻线和居中状态标签的专属状态胶囊，scale 渲染为时间轴虚线网格与刻度标签，事件标注/message/constraint 分别渲染为 marker、track 间箭头与虚线区间标注。`PlantUML gantt` 链路已增强，当前支持 `@startgantt`、`Project starts`、section、task starts/ends/lasts/happens、依赖、resource 轨道与任务颜色，并统一降到 `TimeSeriesIR + GanttLayout` 渲染。`PlantUML wireframe (Salt)` 链路已增强，当前支持 `@startsalt/@endsalt`、基础文本行、`[Button]`、`"placeholder"` / `Label: "placeholder"` 输入框、`^Dropdown^` 下拉简写、`{* A | B | C }` 基础 tab row、`{T ... }` tree、`{+ Title ... }` / `{# Title ... }` / `{frame Title ... }` frame 与 pipe 行 table grid，并通过递归 `WireframeIR + WireframeLayout` 在布局阶段测量容器、子项与表格 cell 尺寸后渲染。✅ `PlantUML chart (pie/bar/line/scatter)` 模块已完成，当前支持 `@startpie/@endpie`、`@startbar/@endbar`、`@startline/@endline`、`@startscatter/@endscatter`、普通 `@startuml` 中 `pie/bar/line/scatter` cue、`title`、pie slice、bar/line/scatter 的 `x-axis` / `y-axis` / 数组 series 与 `Category : value` 行，并复用 `PieIR + PieLayout`、`XYChartIR + XYChartLayout` 渲染饼图、柱状图、折线图与散点图，已接入 `commonTest` 与 one-shot vs chunked 一致性校验。`PlantUML archimate` 链路已增强，当前支持 `archimate #Color "Label" as ID <<stereotype>>`、`group "Layer" as L { ... }`、箭头关系与 `Rel(...)`，并通过 `GraphIR + Sugiyama` 渲染节点与 group cluster。`PlantUML c4 (C4-PlantUML)` 链路已增强，当前支持 C4 include/header、常用元素、boundary、Rel/BiRel/RelIndex/Rel_*、`AddElementTag/AddRelTag` 与 `UpdateElementStyle/UpdateRelStyle` 基础样式宏，并通过 `GraphIR + Sugiyama` 渲染。`PlantUML ditaa` 链路已增强，当前支持 `@startditaa/@endditaa`、ASCII box 识别、简单水平/垂直箭头连接、box 内颜色标记、圆角 corner 字符与 `skinparam handwritten true` 手绘风格，并按原始网格坐标渲染为 `GraphIR`；Phase 5 主要图型已全部启动。
