...
本阶段已完成：`sequence`、`class`、`state`、`component`、`usecase`、`activity`、`object`、`deployment`、`erd`。其中 `sequence` 已补齐常见复杂箭头装饰（`o->` / `->o` / `x->` / `->x` 及其与 `-->>` 等核心箭头的组合）；`component` 已支持 port/package/note 组合布局，排版链路会先布局 primary graph，再统一锚定/避让 port 与 anchored note 这类 decoration，最后重建边路由并重算 cluster 包围盒；`erd` 已支持复杂别名（quoted label / alias）、更宽 crowfoot 关系装饰，以及非标准 attribute 语法（`name type` / `type name`），并均通过 `commonTest` 与 one-shot vs chunked 一致性校验。
