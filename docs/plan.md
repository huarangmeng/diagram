...
### Phase 1 — Mermaid 主力图（Sugiyama 体系） 🚧 进行中
...
当前已落地：
- `flowchart`：lexer / parser / Sugiyama 布局 / streaming sub-pipeline / render；
...
当前状态：⬜ 未开始实现；顶层 API 已能创建 `SourceLanguage.PLANTUML` session，但默认仍走 `StubSessionPipeline`。
