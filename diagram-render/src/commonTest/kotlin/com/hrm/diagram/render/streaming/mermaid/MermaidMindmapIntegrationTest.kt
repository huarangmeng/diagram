...
class MermaidMindmapIntegrationTest {

    @Test
    fun mindmap_official_sample_subset_renders_and_is_streaming_consistent() {
        val src =
            """
            mindmap
              root((mindmap))
    ...
    @Test
    fun mindmap_bang_cloud_and_icon_render_with_both_side_layout() {
        val src =
            """
            mindmap
              root((root))
        ...
        val rootRect = laidOut.nodePositions[ir.root.id] ?: error("missing root")
        val childRects = ir.root.children.map { laidOut.nodePositions[it.id] ?: error("missing child") }

        assertTrue(childRects.any { it.left < rootRect.left }, "expected a left branch: $childRects root=$rootRect")
        assertTrue(childRects.any { it.left > rootRect.left }, "expected a right branch: $childRects root=$rootRect")
        assertTrue(snapshot.drawCommands.any { it is DrawCommand.DrawIcon })
        assertTrue(snapshot.drawCommands.any { it is DrawCommand.FillPath })
    }
