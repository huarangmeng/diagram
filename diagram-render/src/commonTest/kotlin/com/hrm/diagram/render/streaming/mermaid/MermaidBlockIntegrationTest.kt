...
class MermaidBlockIntegrationTest {
    @Test
    fun block_diagram_renders_and_is_streaming_consistent() {
        val src =
            """
            block-beta
              columns 3
    ...
    @Test
    fun block_style_and_class_are_applied() {
        val src =
            """
            block-beta
              columns 2
              ...
              class A hot
              style B fill:#f9f,stroke:#111,color:#123456
            """.trimIndent() + "\n"
    ...
    @Test
    fun block_plain_links_and_unquoted_labels_render_consistently() {
        val src =
            """
            block
              columns 2
        ...
        assertTrue(texts.contains("sync"))
        assertEquals(drawSignature(one.drawCommands), drawSignature(chunked.drawCommands))
    }
