...
class MermaidKanbanIntegrationTest {

    @Test
    fun kanban_official_sample_subset_renders_and_is_streaming_consistent() {
        val src =
            """
            kanban
              todo[Todo]
        ...
        val oneIr = assertIs<KanbanIR>(one.ir)
        val chunkedIr = assertIs<KanbanIR>(chunked.ir)
    ...
    @Test
    fun kanban_ticket_base_url_and_priority_badge_render() {
        val src =
            """
            ---
            config:
        ...
        assertTrue(
            snap.drawCommands.any { it is DrawCommand.Hyperlink && it.href == "https://example.test/browse/MC-2038" },
            "extras=${(snap.ir as KanbanIR).styleHints.extras} draw=${snap.drawCommands}",
        )
        assertTrue(snap.drawCommands.filterIsInstance<DrawCommand.DrawText>().any { it.text == "High" })
