...
- 🟡 Phase 5（已启动）：
  - 🟡 `PlantUML mindmap` 最小可用链路已落地：当前已支持 `*` / `+` / `-` 层级、`+/-` 侧向继承、`: ... ;` 多行节点、stereotype、inline color、前导 `icon/emoji`（如 `<&flag>`、`<:smile:>`、`<#green:sunny:>`），以及 `<style>` 中基于 `BackgroundColor` / `LineColor` / `FontColor` / `RoundCorner` 的 tree style（`.class { ... }` / `.class * { ... }` + `<<class>>`），并完成 `@startmindmap` session 分发、`TreeIR + MindmapLayout` 渲染与 one-shot vs chunked 校验；当前 stereotype 会以独立标题行参与布局与渲染，inline color 已可作用到节点填充/描边/文字，前导 icon 会输出 `DrawIcon`、emoji 当前以 fallback 文本前缀呈现，tree style 已可作用到节点填充、border、edge、标题/正文文字与圆角，更多样式语法与 `wbs` 变体仍待继续实现。
  ...
  - 🟡 `PlantUML network (nwdiag)` 链路已增强：当前已支持 `@startuml` 内 `nwdiag { ... }` 和 `@startnwdiag/@endnwdiag` 专用块、`network name { ... }`、`group name { ... }`、`address = ...`、节点声明、节点属性（`address` / `description` / `label`）以及跨 network 同名节点虚线连接；复用 `GraphIR + Sugiyama` 渲染 network cluster，并将 group 渲染为 nested cluster，已接入 `commonTest` 与 one-shot vs chunked 一致性校验，inet 与形状语义仍待继续实现。
  ...
  - 🟡 `PlantUML gantt` 链路已增强：当前已支持 `@startgantt/@endgantt`、`Project starts`、section `-- Name --`、`[Task] starts/starts at/ends/ends at/lasts/happens at`、`[A] -> [B]` 依赖、`[Task] on {Resource}` 资源轨道与 `[Task] is colored in #Color/Name` 任务颜色；任务会降到 `TimeSeriesIR + GanttLayout` 时间轴渲染，并已接入 `commonTest` 与 one-shot vs chunked 一致性校验，节假日、日历排除、更多任务样式和更完整 PlantUML Gantt 语义仍待继续实现。
  - 🟡 `PlantUML wireframe (Salt)` 链路已增强：当前已支持 `@startsalt/@endsalt`、可选 `salt { ... }` wrapper、基础文本行、完整 `[Button]` 行、`"placeholder"` / `Label: "placeholder"` 输入框、`^Dropdown^` 下拉简写、`{* A | B | C }` 基础 tab row、`{T ... }` tree、`{+ Title ... }` / `{# Title ... }` / `{frame Title ... }` frame，以及 pipe 行 table grid（如 `A | B | C`）；普通文本降为 `WireBox.Plain`，按钮降为 `WireBox.Button`，输入框降为 `WireBox.Input`，基础 tab row 降为 `WireBox.TabbedGroup`，tree/frame/table 均降为嵌套 `WireBox.Plain`，复用递归 `WireframeIR + WireframeLayout` 在布局阶段完成容器、子项与表格 cell 尺寸测量，并已接入 `commonTest` 与 one-shot vs chunked 一致性校验，更完整 Salt 控件语义仍待继续实现。
  - 🟡 `PlantUML chart (pie/bar/line/scatter)` 最小可用链路已落地：当前已支持 `@startpie/@endpie`、`@startbar/@endbar`、`@startline/@endline`、`@startscatter/@endscatter` 专用块、普通 `@startuml` 中的 `pie` / `bar` / `line` / `scatter` cue、`title`、pie slice 行、bar/line/scatter 的 `x-axis` / `y-axis` / `bar [...]` / `line [...]` / `scatter [...]` 数组 series 与 `Category : value` 单系列行；pie 降到共享 `PieIR`，bar/line/scatter 降到共享 `XYChartIR`，渲染层复用 `PieLayout` / `XYChartLayout` 完成标题、图例、坐标轴与标签测量，并输出饼图扇区、柱状图矩形、折线图路径、散点图点标记与坐标网格，已接入 `commonTest` 与 one-shot vs chunked 一致性校验，更完整 chart 样式与其他统计图类型仍待继续实现。
  - 🟡 `PlantUML archimate` 链路已增强：当前已支持 `archimate #Color "Label" as ID <<stereotype>>`、`group "Layer" as L { ... }`、`A --> B : label`、`Rel(A, B, "label")` / `Rel_*`；元素降到 `GraphIR` 并渲染为带 stereotype 与简化图标标记的 ArchiMate 节点，group 降为 `GraphIR.Cluster` 并支持嵌套分层视图，关系支持箭头、标签、虚线，并已接入 `commonTest` 与 one-shot vs chunked 一致性校验，完整 ArchiMate 图标库与 relationship 类型图形语义仍待继续实现。
  - 🟡 `PlantUML c4 (C4-PlantUML)` 链路已增强：当前已支持 `!include/!includeurl` C4 import、`C4Context/C4Container/C4Component/C4Dynamic/C4Deployment` header、`Person/System/Container/Component` 家族、`Boundary/*_Boundary`、`Rel/BiRel/RelIndex/Rel_U/D/L/R/Back`、`AddElementTag/AddRelTag` 与 `UpdateElementStyle/UpdateRelStyle` 基础样式宏；元素降到 `GraphIR` 并渲染为 actor/system/container/component/database 等 C4 样式，boundary 渲染为 cluster，关系 tag/style 可影响线色、线宽与 dashed/dotted 样式，并已接入 `commonTest` 与 one-shot vs chunked 一致性校验，legend、link 交互、deployment node 与更完整 C4-PlantUML 宏仍待继续实现。
  - 🟡 `PlantUML ditaa` 链路已增强：当前已支持 `@startditaa/@endditaa`、`+---+` / `|...|` 与 `/---\` / `\---/` ASCII box 识别、水平 `-->` / `->` / `=>` 连接、基础垂直 `|...v` 连接、box 内颜色标记（如 `cRED` / `cGRE` / `cBLU` / `cYEL` / `cF80`）与 `skinparam handwritten true`；ASCII box 降到 `GraphIR` 节点并按原始网格坐标稳定布局，颜色标记映射为 `NodeStyle.fill`，圆角 corner 字符映射为更大的渲染圆角，手绘风格使用确定性 jitter 的边框与连线，连接线降到 `GraphIR` edge 后以正交线渲染，并已接入 `commonTest` 与 one-shot vs chunked 一致性校验。计划中曾将 ditaa 归入 `StructIR`，本轮未扩公共 IR 契约，先以 `GraphIR` 表达 box/edge；更完整 ASCII art 语义与复杂路由仍待继续实现。
